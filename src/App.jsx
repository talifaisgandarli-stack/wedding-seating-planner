...
export default function WeddingPlanner() {
  ...
  var stats = useMemo(function() {
    ...
    return {
      ...
      oglan: guests.filter(function(g){return g.side==="oglan";}).length,
      qiz: guests.filter(function(g){return g.side==="qiz";}).length,
    };
  }, [guests, tables]);
      ...
      function doSide(side) {
        ...
        groups.forEach(function(entry) {
          ...
          while (seated < members.length) {
            var remaining = members.length - seated;
            var bestT = null;
            var bestScore = -1;
            st.forEach(function(t) {
              var rem = t.cap - used[t.id];
              if (rem <= 0) return;
              var canSeat = Math.min(rem, remaining);
              var score = canSeat === remaining ? rem + 1000 : canSeat;
              if (score > bestScore) { bestT = t; bestScore = score; }
            });
            if (!bestT) break;
            var canSeat = Math.min(bestT.cap - used[bestT.id], remaining);
            for (var i = 0; i < canSeat; i++) {
              members[seated + i].tableId = bestT.id;
              used[bestT.id]++;
            }
            seated += canSeat;
          }
  ...
  var s = function(bg,c,ex) {
    return Object.assign({padding:"10px 18px",background:bg,color:c,border:"none",borderRadius:8,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"system-ui",letterSpacing:0.5},ex||{});
  };
  ...
  return (
    <div style={{fontFamily:"system-ui,sans-serif",height:"100vh",display:"flex",flexDirection:"column",background:"#f4f2ed",overflow:"hidden"}}>
      {/* Top Bar */}
      <div style={{background:"#141414",color:"#fff",padding:"8px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,flexWrap:"wrap",gap:6}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <span style={{fontFamily:"'Playfair Display',serif",fontSize:15}}>Maket Gallery Hall</span>
          <div style={{display:"flex",gap:2,background:"#2a2a2a",borderRadius:6,padding:2}}>
            {[["hall","Zal"],["list","Siyahı"],["stats","Stat"]].map(function(pair){
              return <button key={pair[0]} onClick={function(){setView(pair[0]);}} style={{
                padding:"5px 11px",fontSize:11,border:"none",borderRadius:5,cursor:"pointer",
        ...
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          <span style={{fontSize:11,color:"#888"}}>
            ...
            <span style={{color:"#ccc"}}>{tables.filter(function(t){return t.cap>0;}).length} masa</span>
          </span>
      ...
      {/* Add table toolbar */}
      {addMode && (
        <div style={{background:"#2a6f97",padding:"8px 16px",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",flexShrink:0}}>
          <span style={{color:"#fff",fontSize:12,fontWeight:700}}>🎯 Zalda istədiyin yerə klik et</span>
          <input placeholder={"Ad (məs: №"+(tables.length+1)+")"} value={newTable.label} onChange={function(e){setNewTable(Object.assign({},newTable,{label:e.target.value}));}}
          ...
          <select value={newTable.cap} onChange={function(e){setNewTable(Object.assign({},newTable,{cap:parseInt(e.target.value)}));}}>
            {[8,10,12,14,18,20].map(function(n){return <option key={n} value={n}>{n} nəfərlik</option>;})}
          </select>
          ...
          <button onClick={function(){setAddMode(false);}} style={s("rgba(255,255,255,0.2)","#fff",{padding:"5px 12px",fontSize:11})}>✕ Ləğv et</button>
        </div>
      )}

      <div style={{flex:1,display:"flex",overflow:"hidden"}}>
        {/* LEFT */}
        <div style={{width:230,background:"#fff",borderRight:"1px solid #e0ddd5",display:"flex",flexDirection:"column",flexShrink:0}}>
            ...
            <div style={{display:"flex",gap:3,marginTop:6}}>
              {[["all","Hamısı"],["oglan","Oğlan"],["qiz","Qız"]].map(function(p){
                return <button key={p[0]} onClick={function(){setFSide(p[0]);}} style={{
                  ...
                  background:fSide===p[0]?"#1a1a1a":"#f0f0f0",color:fSide===p[0]?"#fff":"#777"
                }}>{p[1]}</button>;
              })}
            </div>
            {cats.length > 0 && (
          ...
          <div style={{padding:"6px 12px",fontSize:10,color:"#aaa",borderBottom:"1px solid #f5f5f5",fontWeight:600}}>
          ...
          <div style={{flex:1,overflowY:"auto"}}>
            ...
            {filtUn.length === 0 && <div style={{padding:20,textAlign:"center",color:"#ccc",fontSize:11}}>
              {stats.unassigned === 0 ? "Hamısı yerləşib! 🎉" : "Nəticə yoxdur"}
            </div>}
          </div>
        </div>
            ...
            <div style={{padding:"16px 20px",minHeight:"100%"}}>
              <div style={{display:"flex",justifyContent:"center",marginBottom:10,gap:8}}>
                <button onClick={function(){setAddMode(!addMode);setSelTable(null);}}
                  ...
                  {addMode ? "✕ Ləğv et" : "+ Yeni Masa Əlavə Et"}
                </button>
              </div>
              <div ref={hallRef} onClick={handleHallClick}
                ...
                {addMode && <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",fontSize:13,color:"#48bb78",fontWeight:700,opacity:0.4,pointerEvents:"none",zIndex:0,textAlign:"center"}}>
                  Masanı yerləşdirmək üçün<br />klik et
                </div>}
                {tables.map(renderT)}
              </div>
              {activeCats.length > 0 && (
        ...
        {/* RIGHT: Table Detail */}
        {selTable !== null && selTD && (
          <div style={{width:260,background:"#fff",borderLeft:"1px solid #e0ddd5",display:"flex",flexDirection:"column",flexShrink:0}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div>
                  ...
                  <div style={{fontSize:10,color:"#999",marginTop:1}}>
                    {selTD.side==="oglan"?"Oğlan tərəfi":selTD.side==="qiz"?"Qız tərəfi":"Xüsusi"}
                  </div>
                </div>
              ...
              <div style={{display:"flex",gap:6,marginTop:10}}>
                {editTableId === selTable ? (
                  <>
                    ...
                    <button onClick={function(){setEditTableId(null);}}
                      style={{padding:"4px 8px",fontSize:10,background:"#eee",border:"none",borderRadius:4,cursor:"pointer"}}>✕</button>
                  </>
                ) : confirmDel === selTable ? (
                  <div style={{display:"flex",gap:6,alignItems:"center",flex:1}}>
                    <span style={{fontSize:10,color:"#e53e3e",fontWeight:600}}>Silmək istəyirsiniz?</span>
                    <button onClick={function(){deleteTable(selTD.id);}} style={{padding:"4px 10px",fontSize:10,background:"#e53e3e",color:"#fff",border:"none",borderRadius:4,cursor:"pointer",fontWeight:700}}>Bəli, Sil</button>
                  ...
                  <>
                    <button onClick={function(){setEditTableId(selTable);setEtf({label:selTD.label,cap:selTD.cap,side:selTD.side});}}
                      style={{flex:1,padding:"6px 0",fontSize:10,background:"#f0f7ff",color:"#2a6f97",border:"1px solid #d0e3f0",borderRadius:5,cursor:"pointer",fontWeight:600}}>
                      ✎ Düzəlt
                    </button>
                    <button onClick={function(){setConfirmDel(selTable);}}
                      style={{flex:1,padding:"6px 0",fontSize:10,background:"#fff5f5",color:"#e53e3e",border:"1px solid #fdd",borderRadius:5,cursor:"pointer",fontWeight:600}}>
                      🗑 Sil
                    </button>
                  </>
                )}
